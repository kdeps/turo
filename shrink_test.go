...
func TestReduceNeverLargerAndConverges(t *testing.T) {
	...
	for _, level := range []string{"lite", "full", "ultra"} {
		for _, in := range inputs {
			out := reduce(in, level, 0, true, true, true, false, true, false)
			...
			if again := reduce(out, level, 0, true, true, true, false, true, false); estimateTokens(again) > estimateTokens(out) {
